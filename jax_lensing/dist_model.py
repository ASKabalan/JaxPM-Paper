...
from jaxpm.painting import cic_paint_dx, compensate_cic
...
def linear_field(mesh_shape, box_size, pk, field):
    ...
    field = k_field * pkmesh**0.5
    field = ifft3d(field)
    return field


def lpt_lightcone(
    cosmo, initial_conditions, displacement, a, mesh_shape, halo_size=0, sharding=None
):
    """
    Computes first order LPT displacement
    """
    initial_force = pm_forces(
        displacement, mesh_shape, delta=initial_conditions, halo_size=halo_size, sharding=sharding
    )
...
def make_full_field_model(field_size, field_npix, box_shape, box_size, halo_size=0, sharding=None):
    def forward_model(cosmo, nz_shear, initial_conditions):
        ...
        # Create particles
        local_mesh_shape = (*get_local_shape(initial_conditions.shape, sharding), 3)

        print(f"local_shape is {local_mesh_shape}")
        ...
        dx = lpt_lightcone(
            cosmo, lin_field, displacement, a, box_shape, halo_size=halo_size, sharding=sharding
        )

        # Paint the particles on a new mesh
        lightcone = cic_paint_dx(dx, halo_size=halo_size, sharding=sharding)
        ...
        xgrid, ygrid = np.meshgrid(
            np.linspace(0, field_size, box_shape[0], endpoint=False),  # range of X coordinates
        ...
        convergence_maps = [
            kmap.reshape(
                [field_npix, box_shape[0] // field_npix, field_npix, box_shape[1] // field_npix]
            )
            .mean(axis=1)
            .mean(axis=-1)
...
def full_field_probmodel(config):
    ...
    initial_conditions = numpyro.sample(
        "initial_conditions", dist.Normal(jnp.zeros(config.box_shape), jnp.ones(config.box_shape))
    )

...
Porqueres_2023 = config(
    ...
    nz_shear=[
        jc.redshift.kde_nz(
            np.linspace(0.1, 2.5, 100),
            norm.pdf(np.linspace(0.1, 2.5, 100), loc=z_center, scale=0.12),
            bw=0.01,
            zmax=2.5,
            gals_per_arcmin2=g,
        )
...
LSST_Y10 = config(
    ...
    nz_shear=subdivide(
        jc.redshift.smail_nz(2, 0.68, 0.11, gals_per_arcmin2=27, zmax=2.6), nbins=5, zmax=2.5
    ),
    sigma_e=0.26,
    ell_max=3000,
)
...
DES_Y3 = config(
    ...
    nz_shear=[
        jc.redshift.kde_nz(nzs[:, 0], nzs[:, zi + 1], bw=0.01, zmax=3.0, gals_per_arcmin2=g)
